...
if(isset($_POST['submit'])){
    ...
    if(!array_filter($errors)){
        //save to DB
        $query = "INSERT INTO section (codeS, libellé, spécialité) VALUES ($codeS, '$libellé', '$spécialité');";
        $result = mysqli_query($conn, $query);
        if(!$result){
...
?>
    ...
    <div class="whiteContainer column col-sm-11 col-md-10 my-3 px-5 pb-3">
        <h1 id="ajouter_section_header" class="py-4">Ajouter Section</h1>
        <!-- 1er form -->
        <form id="codeS_check_form" action="ajouter_section.php" method="POST" class="ui form m-3">
          <div class="field">
            ...
            <div id="section_inexistante_message" class="ui green message <?php echo $section_inexistante_message_état?>">section inexistante, veuillez saisir le reste des données</div>
            <div id="section_existante_message" class="ui yellow message <?php echo $section_existante_message_état?>">section existante</div>
        ...
        <!-- 2eme form -->
        <form id="add_section_form" action="ajouter_section.php" method="POST" class="ui form m-3">
          ...
          <button id="add_section_btn" type="button" class="ui positive basic button mt-4 inputs_pt2" <?php echo $disabled?> >Ajouter</button>
    ...
    <script>
        ...
        $("#add_section_btn").on('click', () =>{
            ...
            if(spécialité.length == 0){
                $('#spécialité_erreur').text("spécialité manquante");
                erreur_exist = true;
            ...
            if(!erreur_exist){
                Confirm.open({
                    title: 'Confirmation',
                    message: 'Voulez-vous ajouter cette section ?',
                    okText: 'Oui',
                    cancelText: 'Non',
                    onok: () => {
                        var $_POST = {
                        ...
                        ajax_form_submit($_POST, "http://localhost:8080/APP/ajouter_section.php");
                    },
                    oncancel: () =>{
                        Confirm.open({
                            type: 'info',
