...
if(isset($_POST['submit'])){
    ...
    if(!array_filter($errors)){
        //save to DB
        $query = "DELETE FROM examin WHERE matricule = $matricule AND codeM = $codeM;";
        $result = mysqli_query($conn, $query);
        if(!$result){
...
?>
    ...
    <div class="whiteContainer column col-sm-11 col-md-10 my-3 px-5 pb-3">
        <h1 id="supprimer_etudiant_header" class="py-4">Supprimer Examin</h1>
        <!-- 1er form -->
        <form id="check_examin_form" action="supprimer_examin.php" method="POST" class="ui form m-3">
          <div class="field">
          ...
          <div id="examin_existant_message" class="ui green message <?php echo $examin_existant_message_état?>">examin existant.</div>
        </form>
        <!-- séparateur pour le design -->
        <div class="ui divider my-4"></div>
        <!-- 2eme form -->
        <form id="add_student_form" action="supprimer_etudiant.php" method="POST" class="ui form m-3">
          ...
          <div class="field">
            ...
            <input class="inputs_pt2" type="text" name="note" value="<?php echo htmlspecialchars($note); ?>" disabled >
            <p id="note_erreur" class="error_message"><?php echo $errors['note']; ?></p>
          </div>
          <!-- supprimer & annuler boutons -->
          <button id="add_examin_btn" type="button" class="ui positive basic button mt-4 inputs_pt2" <?php echo $disabled?> >Supprimer</button>
    ...
    <script>
        ...
        $("#add_examin_btn").on('click', () =>{
            ...
            Confirm.open({
                title: 'Confirmation',
                message: 'Voulez-vous réellement supprimer cet examin ?',
                okText: 'Oui',
                cancelText: 'Non',
                onok: () => {
                    var $_POST = {
                    ...
                    ajax_form_submit($_POST, "http://localhost:8080/APP/supprimer_examin.php");
                },
                oncancel: () =>{
                    Confirm.open({
                        type: 'info',
