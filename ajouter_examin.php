...
if(isset($_POST['submit'])){
    ...
    if(!array_filter($errors)){
        //save to DB
        $query = "INSERT INTO examin (matricule, codeM, note) VALUES ($matricule, $codeM, $note);";
        $result = mysqli_query($conn, $query);
        if(!$result){
...
?>
    ...
    <div class="whiteContainer column col-sm-11 col-md-10 my-3 px-5 pb-3">
        <h1 id="ajouter_etudiant_header" class="py-4">Ajouter Examin</h1>
        <!-- 1er form -->
        <form id="check_examin_form" action="ajouter_examin.php" method="POST" class="ui form m-3">
          <div class="field">
          ...
          <div id="examin_inexistant_message" class="ui green message <?php echo $examin_inexistant_message_état?>">examin inexistant, veuillez saisir la note</div>
          <div id="examin_existant_message" class="ui yellow message <?php echo $examin_existant_message_état?>">examin existant</div>
        </form>
        <!-- séparateur pour le design -->
        <div class="ui divider my-4"></div>
        <!-- 2eme form -->
        <form id="add_student_form" action="ajouter_etudiant.php" method="POST" class="ui form m-3">
          ...
          <button id="add_examin_btn" type="button" class="ui positive basic button mt-4 inputs_pt2" <?php echo $disabled?> >Ajouter</button>
    ...
    <script>
        ...
        $("#add_examin_btn").on('click', () =>{
            ...
            if(!erreur_exist){
                Confirm.open({
                    title: 'Confirmation',
                    message: 'Voulez-vous ajouter cet examin ?',
                    okText: 'Oui',
                    cancelText: 'Non',
                    onok: () => {
                        var $_POST = {
                        ...
                        ajax_form_submit($_POST, "http://localhost:8080/APP/ajouter_examin.php");
                    },
                    oncancel: () =>{
                        Confirm.open({
                            type: 'info',
