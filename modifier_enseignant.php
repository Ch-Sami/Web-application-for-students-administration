...
if(isset($_POST['submit'])){
    ...
    if(!array_filter($errors)){
        //save to DB
        $query = "UPDATE enseignant SET nomEns = '$nomEns', prénomEns = '$prénomEns', grade = '$grade' WHERE codeEns = $codeEns;";
        $result = mysqli_query($conn, $query);
        if(!$result){
...
?>
    ...
    <div class="whiteContainer column col-sm-11 col-md-10 my-3 px-5 pb-3">
        <h1 id="modifier_enseignant_header" class="py-4">Modifier Enseignant</h1>
        <!-- 1er form -->
        <form id="codeEns_check_form" action="modifier_enseignant.php" method="POST" class="ui form m-3">
          <div class="field">
            ...
            <div id="enseignant_existant_message" class="ui green message <?php echo $enseignant_existant_message_état?>">enseignant existant, veuillez modifier les données</div>
        ...
        <!-- 2eme form -->
        <form id="modify_enseignant_form" action="modifier_enseignant.php" method="POST" class="ui form m-3">
          ...
          <button id="modify_enseignant_btn" type="button" class="ui positive basic button mt-4 inputs_pt2" <?php echo $disabled?> >Modifier</button>
    ...
    <script>
        ...
        $("#modify_enseignant_btn").on('click', () =>{
            ...
            if(!erreur_exist){
                Confirm.open({
                    ...
                    onok: () => {
                        ...
                        ajax_form_submit($_POST, "http://localhost:8080/APP/modifier_enseignant.php");
                    },
                    oncancel: () =>{
                        Confirm.open({
                            type: 'info',
