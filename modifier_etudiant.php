...
if(isset($_POST['submit'])){
    ...
    if(!array_filter($errors)){
        //save to DB
        $query = "UPDATE etudiant SET nom = '$nom', prénom = '$prénom', groupe = '$groupe', codeS = $codeS WHERE matricule = $matricule;";
        $result = mysqli_query($conn, $query);
        if(!$result){
...
?>
    ...
    <div class="whiteContainer column col-sm-11 col-md-10 my-3 px-5 pb-3">
        <h1 id="modifier_etudiant_header" class="py-4">Modifier Etudiant</h1>
        <!-- 1er form -->
        <form id="matricule_check_form" action="modifier_etudiant.php" method="POST" class="ui form m-3">
          <div class="field">
            ...
            <div id="étudiant_existant_message" class="ui green message <?php echo $étudiant_existant_message_état?>">étudiant existant, veuillez modifier les données</div>
        ...
        <!-- 2eme form -->
        <form id="modify_etudiant_form" action="modifier_etudiant.php" method="POST" class="ui form m-3">
          ...
          <button id="modify_etudiant_btn" type="button" class="ui positive basic button mt-4 inputs_pt2" <?php echo $disabled?> >Modifier</button>
            ...
            if(!erreur_exist){
                Confirm.open({
                    ...
                    onok: () => {
                        ...
                        ajax_form_submit($_POST, "http://localhost:8080/APP/modifier_etudiant.php");
                    },
                    oncancel: () =>{
                        Confirm.open({
                            type: 'info',
