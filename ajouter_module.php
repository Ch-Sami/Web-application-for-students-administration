...
if(isset($_POST['submit'])){
    ...
    if(!array_filter($errors)){
        //save to DB
        $query = "INSERT INTO module (codeM, libelléM, coef, codeEns) VALUES ($codeM, '$libelléM', '$coef', '$codeEns');";
        $result = mysqli_query($conn, $query);
        if(!$result){
...
?>
    ...
    <div class="whiteContainer column col-sm-11 col-md-10 my-3 px-5 pb-3">
        <h1 id="ajouter_module_header" class="py-4">Ajouter Module</h1>
        <!-- 1er form -->
        <form id="codeM_check_form" action="ajouter_module.php" method="POST" class="ui form m-3">
          <div class="field">
            ...
            <div id="module_inexistant_message" class="ui green message <?php echo $module_inexistant_message_état?>">module inexistant, veuillez saisir le reste des données</div>
            <div id="module_existant_message" class="ui yellow message <?php echo $module_existant_message_état?>">module existant</div>
        ...
        <!-- 2eme form -->
        <form id="add_module_form" action="ajouter_module.php" method="POST" class="ui form m-3">
          ...
          <button id="add_module_btn" type="button" class="ui positive basic button mt-4 inputs_pt2" <?php echo $disabled?> >Ajouter</button>
    ...
    <script>
        ...
        $("#add_module_btn").on('click', () =>{
            ...
            if(!erreur_exist){
                Confirm.open({
                    title: 'Confirmation',
                    message: 'Voulez-vous ajouter cet module ?',
                    okText: 'Oui',
                    cancelText: 'Non',
                    onok: () => {
                        var $_POST = {
                        ...
                        ajax_form_submit($_POST, "http://localhost:8080/APP/ajouter_module.php");
                    },
                    oncancel: () =>{
                        Confirm.open({
                            type: 'info',
