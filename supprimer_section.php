...
if(isset($_POST['submit'])){
    ...
    //update DB
    $query = "DELETE FROM section WHERE codeS = $codeS;";
    $result = mysqli_query($conn, $query);
    if(!$result){
...
?>
    ...
    <div class="whiteContainer column col-sm-11 col-md-10 my-3 px-5 pb-3">
        <h1 id="supprimer_section_header" class="py-4">Supprimer Section</h1>
        <!-- 1er form -->
        <form id="codeS_check_form" action="supprimer_section.php" method="POST" class="ui form m-3">
          <div class="field">
            ...
            <div id="section_existante_message" class="ui green message <?php echo $section_existante_message_état?>">section existante.</div>
        ...
        <!-- 2eme form -->
        <form id="delete_section_form" action="supprimer_section.php" method="POST" class="ui form m-3">
            <input type="text" class="d-none" name="codeS" value="<?php echo htmlspecialchars($codeS); ?>">
          <!-- libellé -->
          <div class="field">
            <label><span class="label">Libellé</span></label>
            <input class="inputs_pt2" type="text" name="libellé" value="<?php echo htmlspecialchars($libellé); ?>" disabled >
            <p id="libellé_erreur" class="error_message"><?php echo $errors['libellé']; ?></p>
          </div>
          <!-- spécialité -->
          <div class="field">
            <label><span class="label">Spécialité</span></label>
            <input class="inputs_pt2" type="text" name="spécialité" value="<?php echo htmlspecialchars($spécialité); ?>" disabled >
            <p id="spécialité_erreur" class="error_message"><?php echo $errors['spécialité']; ?></p>
          </div>
          <!-- supprimer & annuler boutons -->
          <button id="delete_section_btn" type="button" class="ui positive basic button mt-4 inputs_pt2" <?php echo $disabled?> >Supprimer</button>
    ...
    <script>
        ...
        $("#delete_section_btn").on('click', () =>{
            ...
            Confirm.open({
                title: 'Confirmation',
                message: 'Voulez-vous réellement supprimer cette section ?',
                okText: 'Oui',
                cancelText: 'Non',
                onok: () => {
                    var $_POST = {
                    ...
                    ajax_form_submit($_POST, "http://localhost:8080/APP/supprimer_section.php");
                },
                oncancel: () =>{
                    Confirm.open({
                        type: 'info',
